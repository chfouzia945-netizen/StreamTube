...
async function comment(){if(!token())return location="login.html";let body=comment.value.trim();if(!body)return;await fetch("/api/videos/"+vid+"/comments",{method:"POST",headers:{"Content-Type":"application/json",Authorization:"Bearer "+token()},body:JSON.stringify({body})});comment.value="";loadComments()}
